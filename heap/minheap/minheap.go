...
// NewMaxHeap 初始化一个小根堆结构
func NewMaxHeap[T any](limit int, comparator pkg.Comparator[T]) *MinHeap[T] {
	minHeap := &MinHeap[T]{
		heap:     make([]T, 0),
		cmp:      comparator,
		limit:    limit,
...
// Pop 返回堆中的最大值，并且在小根堆中，把最大值删掉。弹出后依然保持小根堆的结构
...
func down[T any](arr []T, index int, heapSize int, cmp pkg.Comparator[T]) {
	...
	for left < heapSize {
		...
		// 1. 左右孩子的最小值都不小于于当前值，终止寻找。无需继续下沉
		if cmp(arr[smallestIdx], arr[index]) >= 0 { // arr[largestIdx] >= arr[index]
			break
		}
		// 左右孩子的最小值小于当前值
...
func HeapSort[T any](arr []T, comparator pkg.Comparator[T]) {
	...
	// 实例化一个小根堆,此时arr已经是调整后满足小根堆结构的arr
	mh := MinHeap[T]{
		heap:     arr,
		cmp:      comparator,
		limit:    len(arr),
